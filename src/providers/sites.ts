...
export class CoreSitesProvider {
    ...
    siteExists(siteUrl: string): Promise<void> {
        const data: any = {};
        const tokenPath = '/local/mobile_totara/token.php';
        ...
        const promise = this.http.post(siteUrl + tokenPath, data).timeout(CoreConstants.WS_TIMEOUT).toPromise();
    ...
    getUserToken(siteUrl: string, username: string, password: string, service?: string, retry?: boolean)
        ...
        const params = {
            ...
            promise = this.http.post(siteUrl + tokenPath, params).timeout(CoreConstants.WS_TIMEOUT).toPromise();
