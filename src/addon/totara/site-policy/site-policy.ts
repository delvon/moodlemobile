...
import { TotaraLoginProvider } from '../providers/login';
 ...
 */
@IonicPage({ segment: 'addon-totara-site-policy' })
@Component({
    selector: 'page-addon-totara-site-policy',
    templateUrl: 'site-policy.html',
})
export class AddonTotaraSitePolicyPage {
    sitePolicy: any[];
    numPolicies: number;
    showInline: boolean;
    ...
    constructor(
        ...
        navParams: NavParams,
        private totaraLogin: TotaraLoginProvider,
    ...
    protected fetchSitePolicy(): Promise<any> {
        return this.totaraLogin.getSitePolicy(this.siteId).then((sitePolicy) => {
            this.sitePolicy = sitePolicy.policies[0];
            this.numPolicies = sitePolicy.policies.length;
            for (const statement of this.sitePolicy['statements']) {
                this.statementValues[statement.dataid] = null;
            }
    ...
    submit(): void {
        for (const statement of this.sitePolicy['statements']) {
            let value = this.statementValues[statement.dataid];
            if (value === null) {
                this.domUtils.showErrorModal('addon.totara.answerrequired', true);
                return;
            } else if (value === '0' && statement.mandatory) {
                this.domUtils.showErrorModal('addon.totara.consentrequired', true);
                return;
            }
        }

        const modal = this.domUtils.showModalLoading('core.sending', true);
        this.totaraLogin.acceptSitePolicy(this.statementValues, this.siteId).then(() => {
